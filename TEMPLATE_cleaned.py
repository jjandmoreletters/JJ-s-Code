...
# MAGIC %md
# MAGIC ##Template: Cleaned
# MAGIC ##Feed: Dimensional
# MAGIC ##Frequency: Daily
# MAGIC ##Ingestion: Expecting full extraction daily at raw
# MAGIC ##Features: This template can cope with multiple extractions on the same day, and with loading multiple days in one run
...
ingestion_feed_path = "/bimcollab/projects"
...
cleaned_delta_table_name = "cleaned.bimcollabprojects"

# cleaned_delta_table_filepath => used to derive the cleaned path
cleaned_delta_table_filepath = "/bimcollab/projects"
...
schema = StructType([
    StructField("authorization", StructType([
        StructField("project_actions", ArrayType(StringType()) , True)
    ]), True),
    StructField("description", StringType(), False),
    StructField("end_date", TimestampType(), True),
    StructField("extended_data", StringType(), True),
    StructField("features", ArrayType(StringType()), True),
    StructField("max_image_height", LongType(), True),
    StructField("max_image_width", LongType(), True),
    StructField("max_thumbnail_height", LongType(), True),
    StructField("max_thumbnail_width", LongType(), True),
    StructField("name", StringType(), False),
    StructField("owner", StringType(), False),
    StructField("project_id", StringType(), False),
    StructField("project_image_id", StringType(), True),
...
window_start_time = datetime.datetime.strptime(window_start_time,"%Y-%m-%dT%H:%M:%SZ") 
...
if window_end_time == "":
      ...
      window_end_time = datetime.datetime.strptime(window_end_time,"%Y-%m-%dT%H:%M:%SZ")
...
for input_file in dataframe_input_files:
    t = (input_file, datetime.datetime.fromtimestamp(dbutils.fs.ls(input_file)[0].modificationTime/1000))
...
# MAGIC   SELECT * FROM (
# MAGIC       SELECT *, DENSE_RANK() OVER(PARTITION BY ProjectId, __RawPartitionDate ORDER BY __RawFileModificationTime DESC) AS RankSamePartitionDeduplication FROM(
# MAGIC         SELECT
# MAGIC           TRIM(Df.project_id) AS ProjectId
# MAGIC           ,TRIM(Df.name) AS Name
# MAGIC           ,TRIM(Df.`owner`) AS `Owner`
# MAGIC           ,TRIM(Df.`role`) AS `Role`
# MAGIC           ,Df.start_date AS StartDate
# MAGIC           ,Df.end_date AS EndDate
# MAGIC           ,TRIM(Df.description) AS Description
# MAGIC           ,TRIM(Df.extended_data) AS ExtendedData
# MAGIC           ,CAST(Df.authorization AS struct<ProjectActions:array<string>>) AS Authorization
# MAGIC           ,Df.features AS Features
# MAGIC           ,TRIM(Df.project_image_id) AS ProjectImageId
# MAGIC           ,Df.max_image_height AS MaxImageHeight
# MAGIC           ,Df.max_image_width AS MaxImageWidth
# MAGIC           ,Df.max_thumbnail_height AS MaxThumbnailHeight
# MAGIC           ,Df.max_thumbnail_width AS MaxThumbnailWidth
...
CREATE TABLE IF NOT EXISTS {cleaned_delta_table_name}(
  ...
  ,Description string
  ,ExtendedData string
  ,Authorization struct<ProjectActions:array<string>>
  ,Features array<string>
  ,ProjectImageId string
  ,MaxImageHeight long
  ,MaxImageWidth long
  ,MaxThumbnailHeight long
  ,MaxThumbnailWidth long
...
SELECT source.ProjectId
  ...
  ,source.Features
  ,source.ProjectImageId
  ,source.MaxImageHeight
  ,source.MaxImageWidth
  ,source.MaxThumbnailHeight
  ,source.MaxThumbnailWidth
